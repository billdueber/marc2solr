...
module MARC2Solr
  ...
  class Conf
    ...
    OPTIONSCONFIG = [
      ...
      [:printrecord, {:desc =>"Print record (via pp) to --debugfile",
                    :only => [:index],
                    :short => '-r'
                    }],
      [:NOprintrecord, {:desc =>"Turn off printing  Record (via pp) to --debugfile",
                    :only => [:index],
                    }],
      [:printdoc,  {:desc => "Print each completed solr document to --debugfile before sending to solr", 
                    :only => [:index],
                    :short => '-d'}
      ],
